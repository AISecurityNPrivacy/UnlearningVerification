...
def init_model(dataset_name, num_classes, scene, device):
    if scene == 'SCNN':
        i_model = TextSCNN(dataset_name=dataset_name, num_classes=num_classes).to(device)
    elif scene == 'RCNN':
        i_model = TextRCNN(dataset_name=dataset_name, num_classes=num_classes).to(device)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('-scene', '--scenario', type=str, default='basic', choices=['basic', 'SCNN', 'RCNN', 'unbalance'],
                        help='Verify scenario (default: "basic"). Choices: basic, SCNN, RCNN, unbalance')
